...
export default function CreateTableModal({ params }: { params: { restaurantId: string } }) {
   const { id } = useParams() as {id:string};
  const [tables, setTables] = useState<Table[]>([]);
  const [addTableForm, setAddTableForm] = useState(false);
  const [tableNumber, setTableNumber] = useState("");
  const tableArray = [
    { table_number: tableNumber, restaurant_id: id },
  ];
        ...
        {addTableForm && (
          <div className="flex flex-col pt-4 items-center gap-2">
            ...
            <input
              ...
              className="border-2 p-2 rounded-full hover:border-gray-300 hover:scale-105 transition duration-300 hover:bg-green-300 border-gray-300"
            />
